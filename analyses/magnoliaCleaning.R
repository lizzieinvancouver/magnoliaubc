...
# <><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
# From ISSUE #3 April 16

table(allbind$Name)
allbind.sum <- allbind %>%
  group_by(Name,Location) %>%
  summarize()
allbind.sum2 <- allbind.sum %>%
  group_by(Name) %>%
  summarise(n())
# write.csv(allbind.sum2,"numIndividualsPerSpecies.csv")

allbind.yearmin <- allbind %>%
  group_by(Name,Location)%>%
  summarize(min(value))
allbind.yearmax <- allbind %>%
  group_by(Name,Location)%>%
  summarize(max(value))
allbind.yearmerge <- merge(allbind.yearmax,allbind.yearmin) %>%
  mutate(yearmax = year(`max(value)`)) %>%
  mutate(yearmin = year(`min(value)`)) %>%
  mutate(yearlength = yearmax-yearmin)
allbind.yearlength <- allbind.yearmerge %>%
  select(1,2,7)
# write.csv(allbind.yearlength,"magnoliaYearLength.csv")

allbind.obs <- allbind %>%
  group_by(event) %>%
  summarize(n())
write.csv(allbind.obs,"totalObsEvent.csv")
allbind.obs2 <- allbind %>%
  group_by(event,Name,Location) %>%
  summarize(n())
allbind.obs2sum <- allbind.obs2 %>%
  group_by(event,Name) %>%
  summarize(n())
allbind.obs2sum2 <- allbind.obs2sum %>%
  group_by(event) %>%
  summarize(n())
# write.csv(allbind.obs2sum2,"numIndividualsPerEvent.csv")

# Writing the csv that has the cleaned magnolia data
write.csv(allbind, "analyses/output/magnoliaClean.csv")
getwd()
